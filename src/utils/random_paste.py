...
class ObjMap:
    ...
    Attributes:
        ...
        dic_box: the mapping of object box pixels to image coordinates (e.g. {(0, 0):
             np.array(0., 0., 0.)})
        dic: the mapping of object pixels to image coordinates.
        box: the object bounding box in XYXY format.
        """

    def __init__(self, img: NDArray[np.float32], obj: NDArray[np.float32]) -> None:
        self.img = img
        self.obj = obj
        self.dic_box = self.random_location_not_conflict()
        self.dic = self.located_filter_white()
        self.box = self.get_box()

    def get_box(self):
        """
        Get the bounding box with format `XYXY`
        """
        if self.dic_box == {}:
            return None
        else:
            keys = self.dic_box.keys()
            ys, xs = [i[0] for i in keys], [i[1] for i in keys]
    ...
    def located_filter_white(self) -> dict[tuple[int], NDArray[np.float32]]:
        ...
        return {k: v for k, v in self.dic_box.items() if not is_white(v)}
...
def random_paste(
    ...
    return_process_imgs:
        ...
        'mask-box': return [origin_pasted, mask, box];
    ...
    if process_type == "none":
        ...
        return [img_obj, mask]
    elif process_type == "mask-box":
        img_obj = img
        assign_val_based_dict(img_obj, obj_map.dic)
        mask = get_mask(img, obj_map)
        box = obj_map.box
        return [img_obj, mask, box]
...
def random_paste_multi(
    img: NDArray[np.float32], objs: list[NDArray[np.float32]]
) -> list[NDArray[np.float32], NDArray[np.uint8], list[int]]:
    """
    Pastes multiple objects onto an image at random locations.
    ---
    Note: This function maybe can't find a good location, in this situation,
    it's mask will be filled of 0.
    """
    ...
    for obj in objs:
        ...
        img, mask_obj, box = random_paste(img, obj, process_type="mask-box")
        mask += mask_obj * (np.max(mask) + 1)
        if box is not None:
            boxes.append(box)
        print("[INFO]     pasted one obj, good")
    print("[INFO] end random paste multi\n")
    return [img, mask.astype(np.uint8), boxes]
...
if __name__ == "__main__":
    ...
    # Save multi pasted img with boxes
    import torch
    from torchvision.utils import draw_bounding_boxes
    from torchvision.io import write_png
    img_pasted, mask, boxes = random_paste_multi(img, objs)
    img_pasted = torch.Tensor(img_pasted * 255).permute(2, 0, 1).to(torch.uint8)
    boxes = torch.Tensor(boxes)
    img_boxes = draw_bounding_boxes(img_pasted, boxes)
    write_png(img_boxes, "temp.png")
